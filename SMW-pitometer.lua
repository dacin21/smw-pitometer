--#######################################################################
--##                                                                   ##
--##   SMW-Pitometer: A BizHawk lua script for Super Mario World       ##
...
--##                                                                   ##
--##   Special thanks to xHF01x for the rewind counting snippet.       ##
--##                                                                   ##
--##                                                                   ##
--##   Git repository: https://github.com/dacin21/smw-pitometer        ##
--##                                                                   ##
--#######################################################################
...
local INI_CONFIG_NAME = "SMW-pitometer-config.ini"
...
config.DEFAULT_OPTIONS = {
  ...
  hotkey_increase_opacity = "9",  -- to increase the opacity of the text: the '='/'+' key
  hotkey_decrease_opacity = "0",  -- to decrease the opacity of the text: the '_'/'-' key
...
local SMW = {
  ...
  -- Sprites
  sprite_max = HAS_SA1 and 22 or 12,
...
local function sprite_level_info()
  ...
  local status_table = memory.readbyterange(status_table_address.address, 0x80, status_table_address.domain)
...
while true do
  if emu.getsystemid() ~= "SNES" then
    ...
    if HAS_SA1 then
      draw.alert_text(draw.Border_left + draw.Buffer_middle_x*draw.AR_x, draw.Buffer_height*draw.AR_y, "ROM hack uses SA-1, experimental support only.\nReport any issues to the author of this script.", COLOUR.warning, 0, false, 0.5)
    end
  end
